...
async function main(){
  ...
  if(cmd==='launch'){const state=await launch(config,{dryRun:args.includes('--dry-run')});console.log(JSON.stringify(state,null,2));return;}
  if(cmd==='status'){const {StateStore}=await import('./services/state.js');const {printManualFollowUps}=await import('./services/followups.js');const state=await new StateStore().load(config.domain);printManualFollowUps(config,state);console.log(JSON.stringify(state,null,2));return;}
