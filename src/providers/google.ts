...
export class GoogleProvider {
  ...
  async createGa4Property(name:string){
    ...
    const s=await requestJson<{measurementId:string}>(`https://analyticsadmin.googleapis.com/v1beta/${p.name}/dataStreams`,{method:'POST',headers:await this.auth(),body:JSON.stringify({type:'WEB_DATA_STREAM',displayName:name,webStreamData:{defaultUri:`https://${name}`}})});
    return {property:p.name,measurementId:s.measurementId};
  }
  ...
  async getVerificationToken(domain:string){const r=await requestJson<{token:string}>('https://www.googleapis.com/siteVerification/v1/token',{method:'POST',headers:await this.auth(),body:JSON.stringify({site:{type:'INET_DOMAIN',identifier:domain},verificationMethod:'DNS_TXT'})}); return r.token;}
