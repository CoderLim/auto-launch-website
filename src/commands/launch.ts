...
async function resolveProjectDir(config:SiteConfig){
  ...
  try{await stat(candidate);return candidate;}catch{return process.cwd();}
}
export async function launch(config:SiteConfig, opts:{dryRun?:boolean}={}){
  ...
  const siteEnv={AUTO_LAUNCH_DOMAIN:config.domain,AUTO_LAUNCH_PROJECT:config.hosting.projectName};
  await step('prelaunch-audit',async()=>{await run(config.audit?.launchAuditCommand||'',projectDir); await run(config.audit?.buildCommand||'pnpm build',projectDir); return {ok:true};});
  ...
  await step('registrar-ns',()=>config.registrar.provider==='spaceship'
  ...
  await step('hosting-setup',async()=>{await run(config.hosting.setupCommand||'',projectDir,siteEnv);return {ok:true};});
  await step('hosting-deploy',async()=>{if(!config.hosting.deployCommand) throw new AppError('hosting.deployCommand is required for end-to-end launch','MISSING_DEPLOY_COMMAND'); await run(config.hosting.deployCommand,projectDir,siteEnv);return {ok:true};});
  await step('hosting-domain',async()=>{await run(config.hosting.customDomainCommand||'',projectDir,siteEnv);return {ok:true};});  if(config.cloudflare?.alwaysHttps!==false) await step('always-https',()=>cf.setAlwaysHttps(zone!.id,true));
  ...
  if(config.analytics?.ga4){
    ...
    await step('ga4-inject',async()=>{if(!config.analytics?.injectCommand) throw new AppError('analytics.injectCommand is required when analytics.ga4=true','MISSING_GA_INJECT_COMMAND');await run(config.analytics.injectCommand,projectDir,{...siteEnv,AUTO_LAUNCH_GA_ID:ga!.measurementId});return {measurementId:ga!.measurementId};});
    await step('ga4-redeploy',async()=>{await run(config.hosting.deployCommand!,projectDir,{...siteEnv,AUTO_LAUNCH_GA_ID:ga!.measurementId});return {ok:true};});
  }
